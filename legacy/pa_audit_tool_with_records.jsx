...
function useBatchState() {
  ...
  const reset = useCallback(() => {
    setVals(Object.fromEntries(allIds.map(id => [id, null])));
    setNotes(Object.fromEntries(allIds.map(id => [id, ""])));
    setTaskId(""); setQueueName(""); setBatchId(""); setAuditor("");
    setAnnotatorName(""); setAnnotationDate(""); setAuditDate("");
    setExpandedCheck({}); setSubmitted(false); setSubmittedAt(null);
  }, []);
  ...
  return {
    ...
    submitted, submittedAt, setSubmitted, setSubmittedAt, reset,
  };
}
...
// ── Summary view ──────────────────────────────────────────────────────────
function SummaryView({ state, onReset }) {
  return (
    <div>
      {/* Submitted banner */}
      {state.submitted && (
        <div style={{
          ...
          <div style={{ flex: 1, fontSize: 12, color: C.green, fontWeight: 700 }}>
            Audit submitted · {state.submittedAt} · saved to Records
          </div>
          {onReset && (
            <button onClick={onReset} style={{ padding:"6px 12px", borderRadius:6, fontSize:12, fontWeight:700,
              cursor:"pointer", color:C.accent, background:C.accentLo, border:`1px solid ${C.accent}` }}>
              Start new audit
            </button>
          )}
        </div>
...
// ── Main app ──────────────────────────────────────────────────────────────
// ── Records store ─────────────────────────────────────────────────────────
// Three copies of every submitted audit:
//   1. Shared database  — a Google Sheet behind an Apps Script web app. Every auditor reads and writes it,
//                         so the Records tab shows every submission from every device.
//   2. Netlify Forms    — a second inbox in the Netlify dashboard (kept as a backup).
//   3. This browser     — a local cache so the auditor's own submissions never disappear, even offline.
const STORAGE_KEY   = "pa_audit_records_v2";
const SHEET_URL_KEY = "pa_audit_sheet_url";
const NETLIFY_FORM  = "pa-audit";

function loadRecords() {
  try {
    const r = JSON.parse(localStorage.getItem(STORAGE_KEY) || "[]");
    return Array.isArray(r) ? r.filter(x => x && x.record_id) : [];
  } catch { return []; }
}
...
function configuredSheetUrl() {
  const cfg = (typeof window !== "undefined" && window.PA_AUDIT_CONFIG) || {};
  return String(cfg.sheetUrl || "").trim();
}
function getSheetUrl() {
  const baked = configuredSheetUrl();
  if (baked) return baked;
  try { return (localStorage.getItem(SHEET_URL_KEY) || "").trim(); } catch { return ""; }
}
function verdictOf(criticalCount, majorCount) {
  return criticalCount > 0 ? "Hold" : majorCount > 0 ? "Conditional pass" : "Approved";
}
const VERDICT_META = {
  "Approved":         { color:C.green, bg:C.greenLo },
  "Conditional pass": { color:C.amber, bg:C.amberLo },
  "Hold":             { color:C.red,   bg:C.redLo   },
};
...
function buildRecord(state, now) {
  const checks = SECTIONS.flatMap(s => s.checks.map(c => ({
    check_id: c.id.toUpperCase(), section: s.label, severity: c.severity,
    result: state.vals[c.id] ?? "pending", note: state.notes[c.id] || "",
  })));
  const failed   = checks.filter(c => c.result === "fail");
  const critical = failed.filter(c => c.severity === "critical").length;
  const major    = failed.filter(c => c.severity === "major").length;
  const minor    = failed.filter(c => c.severity === "minor").length;
  return {
    record_id: newRecordId(now),
    submitted_at: now.toISOString(),
    task_id: state.taskId, queue_name: state.queueName, batch_id: state.batchId,
    auditor: state.auditor, annotator: state.annotatorName,
    annotation_date: state.annotationDate, audit_date: state.auditDate,
    verdict: verdictOf(critical, major),
    total_checks: checks.length,
    passed: checks.filter(c => c.result === "pass").length,
    failed: failed.length,
    na: checks.filter(c => c.result === "na").length,
    critical_fails: critical, major_fails: major, minor_fails: minor,
    failed_checks: failed.map(c => c.check_id).join(", "),
    // only non-pass checkpoints are kept; a pass carries no note and is implied by the counts
    checks: checks.filter(c => c.result !== "pass"),
    copies: { sheet: null, netlify: null },
  };
}
// Records coming back from the sheet are all text; coerce the numeric fields.
function normalizeShared(r) {
  ...
  return {
    ...
    copies: { sheet: true, netlify: null },
  };
}

// Shared database (Google Apps Script web app)
async function fetchSharedRecords(url) {
  const sep = url.includes("?") ? "&" : "?";
  const r = await fetch(`${url}${sep}action=list&t=${Date.now()}`, { method:"GET", redirect:"follow" });
  if (!r.ok) throw new Error("HTTP " + r.status);
  const data = await r.json();
  if (!data || data.ok !== true) throw new Error((data && data.error) || "Unexpected reply from the shared database");
  return (data.records || []).map(normalizeShared);
}
async function postShared(url, payload) {
  // No content-type header: keeps the request "simple" so the Apps Script redirect works without a preflight.
  const r = await fetch(url, { method:"POST", redirect:"follow", body: JSON.stringify(payload) });
  if (!r.ok) throw new Error("HTTP " + r.status);
  const data = await r.json();
  if (!data || data.ok !== true) throw new Error((data && data.error) || "The shared database rejected the request");
  return data;
}

// Netlify Forms copy
function flatFields(rec) {
  const { checks, copies, ...rest } = rec;
  return { ...rest, checkpoints: JSON.stringify(checks.map(({ check_id, result, note }) => ({ check_id, result, note }))) };
}
function netlifyFormsEnabled() {
  const cfg = (typeof window !== "undefined" && window.PA_AUDIT_CONFIG) || {};
  return cfg.netlifyForms !== false;
}
async function sendToNetlify(rec) {
  if (!isServed() || !netlifyFormsEnabled()) return null;
  try {
    const fields = Object.fromEntries(Object.entries(flatFields(rec)).map(([k, v]) => [k, String(v ?? "")]));
    const body = new URLSearchParams({ "form-name": NETLIFY_FORM, ...fields });
...
function exportRecords(records, format) {
  ...
  const main   = records.map(r => {
    const { checks, copies, ...rest } = r;
    return { ...rest, submitted_at_local: fmtDate(r.submitted_at),
      in_shared_database: copies && copies.sheet === true ? "yes" : "no" };
  });
  const detail = records.flatMap(r => (r.checks || []).map(c => ({ record_id:r.record_id, batch_id:r.batch_id, ...c })));
  if (format === "xlsx" && window.XLSX) {
    ...
    X.writeFile(wb, `pa_audits_${stamp}.xlsx`);
    return;
  }
  const cols = Object.keys(main[0] || {});
  const esc  = v => '"' + String(v ?? "").replace(/"/g, '""') + '"';
  const csv  = [cols.join(","), ...main.map(r => cols.map(c => esc(r[c])).join(","))].join("\r\n");
  const blob = new Blob(["﻿" + csv], { type:"text/csv;charset=utf-8" });
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob); a.download = `pa_audits_${stamp}.csv`; a.click();
  setTimeout(() => URL.revokeObjectURL(a.href), 1000);
}

// ── Records view ──────────────────────────────────────────────────────────
function btnStyle(color, bg, border, disabled) {
  return {
    padding:"7px 14px", borderRadius:6, fontSize:12, fontWeight:700, cursor: disabled ? "default" : "pointer",
    color, background:bg, border:`1px solid ${border}`, opacity: disabled ? .45 : 1,
  };
}

function CopyDot({ ok, label }) {
  const color = ok === true ? C.green : ok === false ? C.amber : C.muted;
  const text  = ok === true ? "✓" : ok === false ? "✕" : "…";
  return (
    <span title={`${label}: ${ok === true ? "saved" : ok === false ? "not saved" : "pending"}`}
      style={{ fontSize:11, color, marginRight:8, whiteSpace:"nowrap" }}>
      {label} {text}
    </span>
  );
}

function SharedSetupCard({ sheetUrl, onConnect, sync }) {
  const [draft, setDraft] = useState(sheetUrl || "");
  const input = { width:"100%", boxSizing:"border-box", padding:"8px 10px", borderRadius:6, fontSize:12,
    background:C.panel, border:`1px solid ${C.border}`, color:C.text, fontFamily:"ui-monospace, Menlo, Consolas, monospace" };
  return (
    <div style={{ background:C.surface, border:`1px solid ${C.accent}`, borderRadius:10, padding:20, marginBottom:16 }}>
      <div style={{ fontSize:14, fontWeight:800, color:C.text }}>Connect the shared database</div>
      <div style={{ fontSize:12, color:C.textDim, marginTop:6, lineHeight:1.55, maxWidth:640 }}>
        Records from every auditor are kept in one shared database. Paste its URL below to connect this browser.
        Setup steps are in the file <span style={{ color:C.text }}>SHARED_DATABASE_SETUP.md</span> that ships with this site.
      </div>
      <div style={{ display:"flex", gap:8, marginTop:14, flexWrap:"wrap" }}>
        <input style={{ ...input, flex:"1 1 360px" }} placeholder="https://script.google.com/macros/s/…/exec"
          value={draft} onChange={e => setDraft(e.target.value)} />
        <button style={btnStyle(C.accent, C.accentLo, C.accent, !draft.trim())} disabled={!draft.trim()}
          onClick={() => onConnect(draft.trim())}>
          {sync.loading ? "Testing…" : "Connect"}
        </button>
      </div>
      {sync.error && (
        <div style={{ fontSize:12, color:C.red, marginTop:10 }}>
          Could not reach the shared database: {sync.error}. Check the URL and try again.
        </div>
      )}
    </div>
  );
...
function RecordsView({ records, sheetUrl, sheetBaked, sync, onConnect, onRefresh, onRetry, onDelete, onClearLocal, onNewAudit }) {
  const [open, setOpen] = useState(null);
  const connected = !!sheetUrl;
  const sorted = [...records].sort((a, b) => String(b.submitted_at || "").localeCompare(String(a.submitted_at || "")));
  const counts = {}; records.forEach(r => { counts[r.verdict] = (counts[r.verdict] || 0) + 1; });
  const unsynced = connected ? records.filter(r => !(r.copies && r.copies.sheet === true)) : [];
  const th = { fontSize:10, fontWeight:700, letterSpacing:".06em", color:C.muted, textAlign:"left",
    padding:"9px 12px", borderBottom:`1px solid ${C.border}`, whiteSpace:"nowrap", textTransform:"uppercase" };
  const td = { fontSize:12, color:C.text, padding:"10px 12px", borderBottom:`1px solid ${C.border}`, verticalAlign:"top", whiteSpace:"nowrap" };
  const none = records.length === 0;

  return (
    <div>
      {!connected && <SharedSetupCard sheetUrl={sheetUrl} onConnect={onConnect} sync={sync} />}
      ...
      <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:20, marginBottom:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:12, alignItems:"flex-start" }}>
          <div style={{ maxWidth:560 }}>
            <div style={{ fontSize:14, fontWeight:800, color:C.text }}>Audit records</div>
            <div style={{ fontSize:11, color:C.textDim, marginTop:4, lineHeight:1.5 }}>
              {connected ? (
                <>
                  {records.length} record{records.length === 1 ? "" : "s"} in the shared database, from every auditor and device.
                  {" "}{sync.loading ? "Refreshing…" : sync.at ? `Last refreshed ${fmtDate(sync.at)}.` : ""}
                  {sync.error && <span style={{ color:C.red }}> Refresh failed: {sync.error}.</span>}
                  {!sheetBaked && <span style={{ color:C.amber }}> Connected on this browser only — bake the URL into index.html so every auditor is connected.</span>}
                </>
              ) : (
                <>{records.length} record{records.length === 1 ? "" : "s"} saved in this browser only. Connect the shared database above to see every auditor's submissions.</>
              )}
            </div>
          </div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {connected && <button style={btnStyle(C.textDim, "transparent", C.border, sync.loading)} disabled={sync.loading} onClick={onRefresh}>↻ Refresh</button>}
            <button style={btnStyle(C.accent, C.accentLo, C.accent, none)} disabled={none} onClick={() => exportRecords(records, "xlsx")}>⬇ Download Excel</button>
            <button style={btnStyle(C.textDim, "transparent", C.border, none)} disabled={none} onClick={() => exportRecords(records, "csv")}>Download CSV</button>
            {!connected && <button style={btnStyle(C.red, "transparent", C.border, none)} disabled={none} onClick={onClearLocal}>Clear all</button>}
          </div>
        </div>
        <div style={{ display:"flex", gap:8, marginTop:14, flexWrap:"wrap", alignItems:"center" }}>
          {Object.entries(VERDICT_META).map(([v, m]) => (
            <span key={v} style={{ fontSize:11, fontWeight:700, color:m.color, background:m.bg, padding:"3px 9px", borderRadius:4 }}>
              {counts[v] || 0} {v}
            </span>
          ))}
          {unsynced.length > 0 && (
            <span style={{ fontSize:11, color:C.amber, marginLeft:"auto" }}>
              {unsynced.length} record{unsynced.length === 1 ? "" : "s"} on this browser not yet in the shared database.{" "}
              <button onClick={onRetry} style={{ ...btnStyle(C.amber, C.amberLo, C.amber, false), padding:"2px 8px" }}>Send now</button>
            </span>
          )}
        </div>
      </div>
      ...
      {none ? (
        <div style={{ background:C.surface, border:`1px dashed ${C.border}`, borderRadius:10, padding:"36px 20px", textAlign:"center" }}>
          <div style={{ fontSize:13, fontWeight:700, color:C.text }}>No audits recorded yet</div>
          <div style={{ fontSize:12, color:C.textDim, marginTop:6 }}>Complete and submit a checklist to add the first record. Records stay here and can be downloaded as Excel.</div>
          <div style={{ display:"flex", gap:8, justifyContent:"center", marginTop:16 }}>
            <button style={btnStyle(C.accent, C.accentLo, C.accent, false)} onClick={onNewAudit}>Start an audit</button>
          </div>
        </div>
      ) : (
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, overflowX:"auto" }}>
          <table style={{ borderCollapse:"collapse", width:"100%", minWidth:1000, fontVariantNumeric:"tabular-nums" }}>
            <thead>
              <tr>
                <th style={th}>Record</th><th style={th}>Batch</th><th style={th}>Task</th><th style={th}>QE</th>
                <th style={th}>Submitted</th><th style={th}>Verdict</th>
                <th style={{ ...th, textAlign:"right" }}>Pass</th><th style={{ ...th, textAlign:"right" }}>Fail</th><th style={{ ...th, textAlign:"right" }}>N/A</th>
                <th style={th}>Crit / Maj / Min</th><th style={th}>Saved to</th><th style={th}></th>
              </tr>
            </thead>
            <tbody>
              {sorted.map(r => {
                const vm = VERDICT_META[r.verdict] || VERDICT_META["Hold"];
                const isOpen = open === r.record_id;
                const issues = (r.checks || []).filter(c => c.result === "fail");
                const copies = r.copies || {};
                return (
                  <React.Fragment key={r.record_id}>
                    <tr>
                      <td style={td}>
                        <div style={{ fontWeight:700, fontFamily:"ui-monospace, Menlo, Consolas, monospace", fontSize:11 }}>{r.record_id}</div>
                      </td>
                      <td style={td}>
                        <div>{r.batch_id || "—"}</div>
                        <div style={{ fontSize:11, color:C.textDim }}>{r.queue_name || ""}</div>
                      </td>
                      <td style={td}>{r.task_id || "—"}</td>
                      <td style={td}>
                        <div>{r.auditor || "—"}</div>
                        <div style={{ fontSize:11, color:C.textDim }}>Annot. {r.annotator || "—"}</div>
                      </td>
                      <td style={td}>{fmtDate(r.submitted_at)}</td>
                      <td style={td}><span style={{ fontSize:11, fontWeight:700, color:vm.color, background:vm.bg, padding:"2px 8px", borderRadius:4 }}>{r.verdict}</span></td>
                      <td style={{ ...td, textAlign:"right", color:C.green }}>{r.passed}</td>
                      <td style={{ ...td, textAlign:"right", color: r.failed ? C.red : C.muted }}>{r.failed}</td>
                      <td style={{ ...td, textAlign:"right", color:C.muted }}>{r.na}</td>
                      <td style={td}>
                        <span style={{ color: r.critical_fails ? C.red : C.muted }}>{r.critical_fails}</span>{" / "}
                        <span style={{ color: r.major_fails ? C.amber : C.muted }}>{r.major_fails}</span>{" / "}
                        <span style={{ color: r.minor_fails ? C.gold : C.muted }}>{r.minor_fails}</span>
                      </td>
                      <td style={td}>
                        {connected && <CopyDot ok={copies.sheet} label="Shared" />}
                        {copies.netlify !== undefined && copies.netlify !== null && <CopyDot ok={copies.netlify} label="Netlify" />}
                        {!connected && (copies.netlify === undefined || copies.netlify === null) && <span style={{ fontSize:11, color:C.muted }}>this browser</span>}
                      </td>
                      <td style={{ ...td, textAlign:"right" }}>
                        <button onClick={() => setOpen(isOpen ? null : r.record_id)} style={{ ...btnStyle(C.accent, "transparent", "transparent", false), padding:"2px 6px" }}>{isOpen ? "Hide" : "Details"}</button>
                        <button onClick={() => onDelete(r)} style={{ ...btnStyle(C.red, "transparent", "transparent", false), padding:"2px 6px" }}>Delete</button>
                      </td>
                    </tr>
                    {isOpen && (
                      <tr>
                        <td colSpan={12} style={{ ...td, whiteSpace:"normal", background:C.panel, padding:"12px 16px" }}>
                          {issues.length === 0 ? (
                            <div style={{ fontSize:12, color:C.green }}>No failed checkpoints. {r.na} marked N/A.</div>
                          ) : (
                            <div style={{ display:"grid", gap:8 }}>
                              {issues.map(c => (
                                <div key={c.check_id} style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                                  <SevChip sev={SEV_META[c.severity] ? c.severity : "minor"} />
                                  <div style={{ fontSize:12 }}>
                                    <span style={{ fontWeight:700, fontFamily:"ui-monospace, Menlo, Consolas, monospace" }}>{c.check_id}</span>
                                    <span style={{ color:C.textDim }}> · {c.section}</span>
                                    <div style={{ color: c.note ? C.text : C.muted, marginTop:2 }}>{c.note || "No note recorded."}</div>
                                  </div>
                                </div>
                              ))}
                            </div>
...
export default function PAQualityAudit() {
  ...
  const [local, setLocal]       = useState(() => loadRecords());   // this browser's cache
  const [shared, setShared]     = useState(null);                  // null until the sheet has been read once
  const [sheetUrl, setSheetUrl] = useState(() => getSheetUrl());
  const [sync, setSync]         = useState({ loading:false, error:null, at:null });
  const sheetBaked  = !!configuredSheetUrl();
  const totalChecks = SECTIONS.reduce((a, s) => a + s.checks.length, 0);

  const persistLocal = useCallback(next => { setLocal(next); saveRecords(next); }, []);
  const patchLocal = useCallback((id, patch) => {
    setLocal(prev => {
      const next = prev.map(r => r.record_id === id ? { ...r, ...patch, copies: { ...(r.copies || {}), ...(patch.copies || {}) } } : r);
      saveRecords(next);
      return next;
    });
  }, []);

  const refresh = useCallback(async (url = sheetUrl) => {
    if (!url) return false;
    setSync(s => ({ ...s, loading:true, error:null }));
    try {
      const rows = await fetchSharedRecords(url);
      setShared(rows);
      setSync({ loading:false, error:null, at: new Date().toISOString() });
      return true;
    } catch (e) {
      setSync(s => ({ ...s, loading:false, error: e.message || String(e) }));
      return false;
    }
  }, [sheetUrl]);

  // Read the shared database on first load and whenever the Records tab is opened.
  useEffect(() => { if (sheetUrl) refresh(); }, [sheetUrl]);
  useEffect(() => { if (view === "records" && sheetUrl) refresh(); }, [view]);

  async function pushToSheet(rec, url = sheetUrl) {
    if (!url) return;
    try {
      await postShared(url, { action:"add", record: rec });
      patchLocal(rec.record_id, { copies: { sheet: true } });
      setShared(prev => {
        if (!prev) return prev;
        if (prev.some(r => r.record_id === rec.record_id)) return prev;
        return [{ ...rec, copies: { ...(rec.copies || {}), sheet: true } }, ...prev];
      });
    } catch {
      patchLocal(rec.record_id, { copies: { sheet: false } });
    }
  }
  ...
  async function handleSubmit() {
    ...
    const rec = buildRecord(state, now);
    persistLocal([rec, ...local]);
    // auto-navigate to summary so QE sees the verdict immediately
    setView("summary");
    pushToSheet(rec);
    sendToNetlify(rec).then(ok => { if (ok !== null) patchLocal(rec.record_id, { copies: { netlify: ok } }); });
  }
  ...
  async function handleConnect(url) {
    ...
    if (!ok) {
      // keep the URL out of the way until it actually works
      try { localStorage.removeItem(SHEET_URL_KEY); } catch {}
      setSheetUrl("");
    }
  }

  function handleRetry() {
    local.filter(r => !(r.copies && r.copies.sheet === true)).forEach(r => pushToSheet(r));
  }

  async function handleDelete(rec) {
    const inSheet = sheetUrl && shared && shared.some(r => r.record_id === rec.record_id);
    if (inSheet) {
      if (!window.confirm(`Delete ${rec.record_id} from the shared database for everyone?`)) return;
      try {
        await postShared(sheetUrl, { action:"delete", record_id: rec.record_id });
        setShared(prev => prev ? prev.filter(r => r.record_id !== rec.record_id) : prev);
      } catch (e) {
        window.alert("Could not delete from the shared database: " + (e.message || e));
        return;
      }
    }
    persistLocal(local.filter(r => r.record_id !== rec.record_id));
  }
  function handleClearLocal() {
    if (window.confirm(`Delete all ${local.length} audit records saved in this browser?`)) persistLocal([]);
  }
  function handleNewAudit() { state.reset(); setView("audit"); window.scrollTo(0, 0); }

  // What the Records tab shows: the shared database, plus anything on this browser that isn't in it yet.
  const records = (() => {
    if (!sheetUrl || !shared) return local;
    const ids = new Set(shared.map(r => r.record_id));
    return [...shared, ...local.filter(r => !ids.has(r.record_id))];
  })();
  ...
  const tabs = [
    ...
    { id:"records",   label: records.length ? `Records (${records.length})` : "Records" },
  ];
  ...
  return (
    <div style={{ fontFamily:"'Inter', system-ui, -apple-system, sans-serif",
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
        ...
        {view === "summary"   && <SummaryView state={state} onReset={handleNewAudit} />}
        {view === "records"   && (
          <RecordsView records={records} sheetUrl={sheetUrl} sheetBaked={sheetBaked} sync={sync}
            onConnect={handleConnect} onRefresh={() => refresh()} onRetry={handleRetry}
            onDelete={handleDelete} onClearLocal={handleClearLocal} onNewAudit={handleNewAudit} />
        )}
      </div>
    </div>
  );
...
createRoot(document.getElementById("root")).render(<PAQualityAudit />);
